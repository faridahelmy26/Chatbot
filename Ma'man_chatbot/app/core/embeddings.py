...
try:
    ...
    print(f"⚠️ Error importing fastembed: {e}")
    raise
...
class EmbeddingEngine:
    ...
    def build_embeddings(self):
        ...
        raw_embeddings = list(self.model.embed(sentences))
        embeddings = np.array([_normalize(np.array(e)) for e in raw_embeddings])
    ...
    def search(self, query):
        ...
        try:
                query_embedding = np.array([_normalize(np.array(e)) for e in self.model.embed([query])])
